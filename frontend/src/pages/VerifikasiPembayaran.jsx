...
export default function VerifikasiPembayaran() {
  ...
  const verifikasiList = MOCK_TAGIHAN.filter(
    (t) => t.status === "Menunggu Konfirmasi" || t.status === "Approved"
  );
                ...
                <tbody className="font-sans text-base">
                  {verifikasiList.map((inv, index) => (
                    <tr key={inv.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${index % 2 !== 0 ? 'bg-gray-50/50' : ''}`}>
                      ...
                      <td className="py-4 px-6 text-gray-800">{formatTanggal(inv.tglDibayar)}</td>
          ...
          <div className="bg-surface p-6 flex flex-col gap-6 relative">
            ...
            {selectedInvoice.status === "Menunggu Konfirmasi" ? (
              <div className="flex gap-3 pt-4 border-t border-gray-100">
                <button onClick={closeModal} className="flex-1 py-3 bg-success hover:bg-green-700 text-white font-bold rounded-full transition-colors cursor-pointer">
                  Approve
                </button>
                <button onClick={closeModal} className="flex-1 py-3 bg-danger hover:bg-red-700 text-white font-bold rounded-full transition-colors cursor-pointer">
                  Disapprove
                </button>
              </div>
            ) : (
